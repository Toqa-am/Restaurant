  ...
  useEffect(
    () => {
      if (pokemon.pokemon.meals) {
        ...
        total += pokemon.pokemon.meals.reduce(
          (acc, meal) => acc + meal.smallest_price,
          0
        );
      }

      if (pokemon.extras) {
        total += pokemon.extras.reduce(
          (acc, extra) => acc + extra.smallest_price,
          0
        );
      }

      if (pokemon.addons) {
        total += pokemon.addons.reduce(
          (acc, addon) => acc + addon.smallest_price,
          0
        );
      }
      setTotal(total);
      ...
      <div key={pokemon.pokemon.id} className="card mb-3 grid-card ">
        ...
        <div class="card-body ">
          <span className="d-flex justify-content-between align-items-center justify-content-center item-card">
            ...
            <span class="badge bg-danger mb-10">
              {" "}
              -{pokemon.pokemon.discount}{" "}
            </span>
          </span>
          <div className="">
            ...
            <span className="d-flex justify-content-between align-items-center justify-content-center item-card">
              <p className="price text-muted text-decoration-line-through">
                {" "}
                OMR {pokemon.pokemon.total_price_before_discount}
              </p>
            ...
            <div className="d-flex justify-content-between align-items-center justify-content-center item-card">
              <p className="price">
                {" "}
                OMR {pokemon.pokemon.total_price_after_discount}
              </p>
                  ...
                  <div className="modal-content">
                    ...
                    <div className="modal-body">
                      <CartCard
                        ...
                        price={pokemon.pokemon.total_price_after_discount}
                        quant={itemQuant}
                        increase={() => increaseItems(pokemon.pokemon)}
                        decrease={() => decreaseItems(pokemon.pokemon)}
                      />
                    </div>
                    <div className="modal-footer">
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleAddToCart(pokemon.pokemon, itemQuant)
                        }
                        data-bs-dismiss="modal"
                      >
                        Add to cart
